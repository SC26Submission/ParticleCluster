...
class PlyProperty {
 public:
  ...
  void ReserveData(int64_t num_entries) {
    data_.reserve(static_cast<int64_t>(DataTypeLength(data_type_)) * num_entries);
  }
  ...
  const void *GetDataEntryAddress(int entry_id) const {
    return data_.data() + static_cast<int64_t>(entry_id) * data_type_num_bytes_;
  }
