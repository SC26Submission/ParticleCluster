...
static void ComputeBitLengths(const uint32_t *symbols, int64_t num_values,
                              int num_components,
  ...
  *out_max_value = 0;
  ...
  for (int64_t i = 0; i < num_values; i += num_components) {
...
static int64_t ApproximateRawSchemeBits(const uint32_t *symbols,
  ...
  int num_unique_symbols;
  // ComputeShannonEntropy takes int; clamp to avoid overflow in estimate.
  const int num_symbols_clamped =
      static_cast<int>(std::min(num_symbols, (int64_t)INT_MAX));
  const int64_t data_bits = ComputeShannonEntropy(
      symbols, num_symbols_clamped, max_value, &num_unique_symbols);
...
bool EncodeTaggedSymbols(const uint32_t *symbols, int64_t num_values,
                         int num_components,
                         const std::vector<uint32_t> &bit_lengths,
                         EncoderBuffer *target_buffer);

template <template <int> class SymbolEncoderT>
bool EncodeRawSymbols(const uint32_t *symbols, int64_t num_values,
                      uint32_t max_entry_value, int32_t num_unique_symbols,
                      const Options *options, EncoderBuffer *target_buffer);

bool EncodeSymbols(const uint32_t *symbols, int64_t num_values,
                   int num_components, const Options *options,
                   EncoderBuffer *target_buffer) {
  if (num_values < 0) {
    return false;
  }
...
bool EncodeTaggedSymbols(const uint32_t *symbols, int64_t num_values,
  ...
  if (tag_encoder.needs_reverse_encoding()) {
    ...
    for (int64_t i = num_values - num_components; i >= 0; i -= num_components) {
      ...
      const int64_t j = num_values - num_components - i;
...
bool EncodeRawSymbolsInternal(const uint32_t *symbols, int64_t num_values,
