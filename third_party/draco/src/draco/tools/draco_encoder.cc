...
int main(int argc, char **argv) {
  ...
  if (!options.is_point_cloud) {
    ...
    // Inject original-index attribute so decoded output can be reordered.
    draco::GeometryAttribute index_attr;
    index_attr.Init(draco::GeometryAttribute::GENERIC, nullptr, 1,
                    draco::DT_INT32, false, sizeof(int32_t), 0);
    const int index_att_id =
        pc->AddAttribute(index_attr, true, pc->num_points());
    for (draco::PointIndex i(0); i < pc->num_points(); ++i) {
      const int32_t idx = i.value();
      pc->attribute(index_att_id)->SetAttributeValue(
          draco::AttributeValueIndex(i.value()), &idx);
    }
  }
