...
int main(int argc, char **argv) {
  ...
  // Export original index mapping if GENERIC attribute exists (added by encoder).
  if (pc->NumNamedAttributes(draco::GeometryAttribute::GENERIC) > 0) {
    const draco::PointAttribute *index_attr =
        pc->GetNamedAttribute(draco::GeometryAttribute::GENERIC, 0);
    const std::string order_file = options.output + ".order";
    std::vector<int32_t> order(pc->num_points());
    for (draco::PointIndex i(0); i < pc->num_points(); ++i) {
      int32_t val;
      index_attr->GetValue(index_attr->mapped_index(i), &val);
      order[i.value()] = val;
    }
    FILE *fp = fopen(order_file.c_str(), "wb");
    if (fp) {
      fwrite(order.data(), sizeof(int32_t), order.size(), fp);
      fclose(fp);
      printf("Original index order saved to %s\n", order_file.c_str());
    }
  }
