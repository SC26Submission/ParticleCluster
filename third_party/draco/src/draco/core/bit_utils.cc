...
void ConvertSignedIntsToSymbols(const int32_t *in, int64_t in_values,
                                uint32_t *out) {
  ...
  for (int64_t i = 0; i < in_values; ++i) {
    out[i] = ConvertSignedIntToSymbol(in[i]);
  }
}

void ConvertSymbolsToSignedInts(const uint32_t *in, int64_t in_values,
                                int32_t *out) {
  for (int64_t i = 0; i < in_values; ++i) {
