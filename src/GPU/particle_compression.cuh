...
template <typename T> struct CompressionState2D {
  ...
  unsigned long long num_candidate_vulnerable_pairs = 0;
  bool active_pairs_already_filtered = false;
  bool pair_counts_reported = false;
  ...
  void free() {
    ...
    d_signs = nullptr;
    num_candidate_vulnerable_pairs = 0;
    N_local = 0;
    active_pairs_already_filtered = false;
    pair_counts_reported = false;
  }
};
...
__device__ __forceinline__ bool unionPairScanMinChanged(int *parent, int x,
                                                        int y) {
  while (true) {
    x = findPairScanRootReadonly(parent, x);
    y = findPairScanRootReadonly(parent, y);
...
appendVulnerablePair(int pt_idx1, int pt_idx2, bool sign,
                     ...
                     unsigned long long *d_num_vulnerable_pairs,
                     int local_pairs[][2], bool *local_signs,
                     int &num_local_pair, int max_pairs) {
  ...
  if constexpr (CountOnly) {
    atomicAdd(d_num_vulnerable_pairs,
              static_cast<unsigned long long>(num_local_pair));
  } else {
    unsigned long long base_pos =
        atomicAdd(d_num_vulnerable_pairs,
                  static_cast<unsigned long long>(num_local_pair));
    unsigned long long available =
        base_pos < static_cast<unsigned long long>(max_pairs)
            ? static_cast<unsigned long long>(max_pairs) - base_pos
            : 0ULL;
    unsigned long long capped_available =
        available < 2147483647ULL ? available : 2147483647ULL;
    int write_count = min(num_local_pair, static_cast<int>(capped_available));
    for (int j = 0; j < write_count; ++j) {
      int out_idx = static_cast<int>(base_pos) + j;
      d_vulnerable_pairs[out_idx] =
          make_int2(local_pairs[j][0], local_pairs[j][1]);
      d_signs[out_idx] = local_signs[j];
    }
  }
  num_local_pair = 0;
}
...
flushVulnerablePairs(int2 *d_vulnerable_pairs, bool *d_signs,
  ...
  if constexpr (CountOnly) {
    ...
    for (int i = 0; i < write_count; ++i) {
      int out_idx = static_cast<int>(base_pos) + i;
      d_vulnerable_pairs[out_idx] =
          make_int2(local_pairs[i][0], local_pairs[i][1]);
      d_signs[out_idx] = local_signs[i];
    }
  }
  num_local_pair = 0;
}
...
__global__ void findVulnerablePairsCellPairs2D_kernel(
    ...
    int2 *d_vulnerable_pairs, bool *d_signs,
    unsigned long long *d_num_vulnerable_pairs, int N, T lower_bound_sq,
    T upper_bound_sq, T sign_bound_sq, int *d_safe_roots, int *d_halo_roots,
    T safe_bound_sq, T halo_bound_sq, int max_pairs, int N_local = 0) {
  if (N_local == 0)
    N_local = N;
...
template <bool CountOnly, bool ContractedHaloForest, typename T>
__global__ void findActivePairsCellPairs2D_kernel(
    const T *__restrict__ d_org_xx, const T *__restrict__ d_org_yy,
    const int *d_cell_start, const int *d_cell_pts_sorted,
    const int2 *d_cell_pairs, int num_cell_pairs, int num_cells,
    int2 *d_active_pairs, bool *d_signs, unsigned long long *d_num_active_pairs,
    int N, T lower_bound_sq, T upper_bound_sq, T sign_bound_sq,
    const int *d_safe_roots, const int *d_halo_roots, int *d_supernode_parent,
  ...
  for (long long k = threadIdx.x; k < total_pairs; k += blockDim.x) {
    ...
    bool cross_process_pair = (pt_idx1 < N_local) != (pt_idx2 < N_local);
    ...
    bool sign = dist_sq > sign_bound_sq;
    bool keep = cross_process_pair;
    if constexpr (ContractedHaloForest) {
      if (!keep) {
        int safe_p = d_safe_roots[pt_idx1];
        int safe_q = d_safe_roots[pt_idx2];
        if (sign) {
          keep = d_halo_roots[pt_idx1] != d_halo_roots[pt_idx2];
        } else if (safe_p != safe_q) {
          keep = unionPairScanMinChanged(d_supernode_parent, safe_p, safe_q);
        }
      }
    } else {
      if (!keep)
        keep = d_safe_roots[pt_idx1] != d_safe_roots[pt_idx2];
    }

    if (keep) {
      appendVulnerablePair<CountOnly>(pt_idx1, pt_idx2, sign, d_active_pairs,
                                      d_signs, d_num_active_pairs, local_pairs,
                                      local_signs, num_local_pair, max_pairs);
    }
  }

  flushVulnerablePairs<CountOnly>(d_active_pairs, d_signs, d_num_active_pairs,
                                  local_pairs, local_signs, num_local_pair,
                                  max_pairs);
}

template <bool CountOnly, bool ContractedHaloForest, typename T>
__global__ void findActivePairsCellPairs3D_kernel(
    const T *__restrict__ d_org_xx, const T *__restrict__ d_org_yy,
    const T *__restrict__ d_org_zz, const int *d_cell_start,
    const int *d_cell_pts_sorted, const int2 *d_cell_pairs, int num_cell_pairs,
    int num_cells, int2 *d_active_pairs, bool *d_signs,
    unsigned long long *d_num_active_pairs, int N, T lower_bound_sq,
    T upper_bound_sq, T sign_bound_sq, const int *d_safe_roots,
    const int *d_halo_roots, int *d_supernode_parent, int max_pairs,
    int N_local = 0) {
  if (N_local == 0)
    N_local = N;
...
template <typename T>
__global__ void
countPGDViolations2D_kernel(const int2 *vulnerable_links, const bool *signs,
                            const T *decomp_x, const T *decomp_y, T b, T xi,
                            int num_pairs, unsigned long long *count_buffer,
                            T decomp_tol) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  extern __shared__ unsigned char violation_sdata_raw_2d[];
  unsigned int *sdata =
      reinterpret_cast<unsigned int *>(violation_sdata_raw_2d);
  int tid = threadIdx.x;
  ...
  if (tid == 0 && sdata[0] > 0)
    atomicAdd(count_buffer, static_cast<unsigned long long>(sdata[0]));
}

template <typename T>
__global__ void
countPGDViolations3D_kernel(const int2 *vulnerable_links, const bool *signs,
                            const T *decomp_x, const T *decomp_y,
                            const T *decomp_z, T b, T xi, int num_pairs,
                            unsigned long long *count_buffer, T decomp_tol) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int local_count = 0;
  ...
  extern __shared__ unsigned char violation_sdata_raw_3d[];
  unsigned int *sdata =
      reinterpret_cast<unsigned int *>(violation_sdata_raw_3d);
  int tid = threadIdx.x;
  sdata[tid] = local_count;
  __syncthreads();
...
__global__ void compactCellCompressionOutputs_kernel(
    ...
    UInt *d_quant_codes, T *d_lossless_values, int total_quant_codes,
    int total_lossless_values, unsigned long long *d_error_count) {
  int cell_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (cell_id >= num_cells)
    return;
  ...
  if (cell_start < 0 || cell_end < cell_start || cell_end > N) {
    atomicAdd(d_error_count, 1ULL);
    return;
  }

  if (n == 0) {
    if (qoff < 0 || qoff >= total_quant_codes) {
      atomicAdd(d_error_count, 1ULL);
      return;
    }
  ...
  for (int pos = cell_start; pos < cell_end; ++pos) {
    for (int coord = 0; coord < D; ++coord) {
      if (d_lossless_flag[D * pos + coord]) {
        if (loff < 0 || loff >= total_lossless_values ||
            d_lossless_values == nullptr) {
          atomicAdd(d_error_count, 1ULL);
          return;
        }
