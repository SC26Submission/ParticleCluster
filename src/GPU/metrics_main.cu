...
struct MetricOptions {
  ...
  bool compute_ari = true;
  bool compute_iou = true;
  double b = 0.0;
  double linking_parameter = 0.2;
};

double target_cell_occupancy =
    0; // 0 means finest dense grid up to INT_MAX cells
...
static void parseError(const char *error) {
  ...
  std::fprintf(stderr,
               ...
               "[-occ <n>] [--metrics <all|ari|iou|stats>]\n");
  std::fprintf(stderr,
               "  Metric mode aliases: --all, --ari-only, --iou-only, "
               "--stats-only. --no-iou keeps the old stats+ARI behavior.\n");
  std::fprintf(stderr,
...
static void setMetricMode(MetricOptions &opts, const std::string &mode) {
  if (mode == "all") {
    opts.compute_stats = true;
    opts.compute_ari = true;
    opts.compute_iou = true;
  } else if (mode == "ari" || mode == "ari-only") {
    opts.compute_stats = false;
    opts.compute_ari = true;
    opts.compute_iou = false;
  } else if (mode == "iou" || mode == "iou-only") {
    opts.compute_stats = false;
    opts.compute_ari = false;
    opts.compute_iou = true;
  } else if (mode == "stats" || mode == "stat" || mode == "statistics" ||
             mode == "stats-only" || mode == "stat-only" ||
             mode == "statistics-only") {
    opts.compute_stats = true;
    opts.compute_ari = false;
    opts.compute_iou = false;
  } else {
    parseError("Metric mode must be one of: all, ari, iou, stats");
  }
}
...
static MetricOptions parseArgs(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (arg == "--metrics" || arg == "--metric" ||
               arg == "--metric-mode" || arg == "-m") {
      if (i + 1 >= argc)
        parseError("Missing metric mode after --metrics");
      setMetricMode(opts, argv[++i]);
    } else if (arg == "--all") {
      setMetricMode(opts, "all");
    } else if (arg == "--ari-only" || arg == "-ari-only") {
      setMetricMode(opts, "ari");
    } else if (arg == "--iou-only" || arg == "-iou-only") {
      setMetricMode(opts, "iou");
    } else if (arg == "--stats-only" || arg == "--stat-only" ||
               arg == "--statistics-only" || arg == "-stats-only" ||
               arg == "-stat-only") {
      setMetricMode(opts, "stats");
    } else if (arg == "--no-iou" || arg == "-no-iou" || arg == "--skip-iou" ||
               arg == "-skip-iou") {
      opts.compute_stats = true;
      opts.compute_ari = true;
      opts.compute_iou = false;
...
template <typename T> static void run2D(const MetricOptions &opts) {
  ...
  if (opts.compute_ari || opts.compute_iou) {
    computeFoFRoots2D(d_org_x, d_org_y, min_x, range_x, min_y, range_y, n_int,
                      b, &d_org_roots);
    ...
    if (opts.compute_iou) {
      ...
      CUDA_CHECK(cudaMemcpy(dec_roots.data(), d_dec_roots, N * sizeof(int),
                            cudaMemcpyDeviceToHost));
      iou = calculateIoUStats(org_roots, dec_roots);
    }
  }
...
template <typename T> static void run3D(const MetricOptions &opts) {
  ...
  PointwiseStats pointwise;
  if (opts.compute_stats) {
    pointwise = computePointwiseStats3D(org_x, org_y, org_z, dec_x, dec_y,
                                        dec_z, range_x, range_y, range_z);
  }
